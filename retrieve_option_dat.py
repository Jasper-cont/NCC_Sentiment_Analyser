...
def calculate_put_call_ratio(options_data, options):
    ...
    for expiry_date in options:
        ...
        if calls_vol > 0:
            temp_data['putCallRatio'].append( puts_vol / calls_vol )
        else:
            temp_data['putCallRatio'].append( 0 )
...
merged_options_data = pd.merge(put_call_ratio_data, open_interest, on='expirationDate', how='outer')

merged_options_data = pd.merge(merged_options_data, average_iv_by_date, on='expirationDate', how='outer')

print(merged_options_data)